...
public class GameGrain : Grain, IGameGrain
{
    ...
    public List<Guid> _playerIds;
    private int indexNextPlayerToMove;

    public GameState _gameState;
    public Guid _winnerId;
    public Guid _loserId;
    ...
    public Task<GameState> AddPlayerToGame(Guid player)
    {
        ...
        if (_gameState == GameState.Finished) throw new ApplicationException("Can't join game once its over");
        if (_gameState == GameState.InPlay) throw new ApplicationException("Can't join game once its in play");
    ...
    public async Task<GameState> MakeMove(GameMove move)
    {
        ...
        if (_playerIds.IndexOf(move.PlayerId) < 0)
            throw new ArgumentException("No such playerid for this game", "move");
        if (move.PlayerId != _playerIds[indexNextPlayerToMove])
